...
def network_ode(_time, state, robot_parameters, loads, contact_sens):
    ...
    feedback = robot_parameters.feedback
    ...
    sigma = robot_parameters.sim_parameters.weights_contact_limb_i
    contact_sens=np.asarray(contact_sens)
    contact_forces=[sigma * contact_sens[i] for i in range(4)]
    ...
    if feedback:
        for i in range(16, 20):
            dtheta[i] +=contact_forces[i-16]*np.cos(phases[i])
